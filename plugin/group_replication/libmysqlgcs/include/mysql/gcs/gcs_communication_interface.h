...
/**
  ...
  它是面向组的，这意味着每个组应该存在一个实例，因为它是由Gcs_interface:：get_communication_session类返回的。
  ...
  - Message receiving: Due to the asynchronous nature of this interface,
    ...
    消息接收：由于此接口的异步特性，消息接收是通过事件完成的。
    它们应以Gcs_communication_event_listener类中定义的回调形式进行传递，
    该回调应由对接收这些通知感兴趣的接口的客户端实现。
