...
static int skip_msg(pax_msg *p) {
  ...
  p->msg_type = no_op;
  //printf("skipping, %lld,   %lld\n",(long long)p->synode.msgno, (long long)p->synode.node);
  //fflush(stdout);
...
static void push_msg_3p(site_def const *site, pax_machine *p, pax_msg *msg,
  ...
  //printf(" before ---- p cnt: %lld, p node: %lld, m cnt: %lld, m node: %lld\n", (long long)p->proposer.bal.cnt, (long long)p->proposer.bal.node, (long long)msg->reply_to.cnt, (long long)msg->reply_to.node);
  prepare_push_3p(site, p, msg, msgno, msg_type);
  //printf(" after ---- p cnt: %lld, p node: %lld, m cnt: %lld, m node: %lld\n", (long long)p->proposer.bal.cnt, (long long)p->proposer.bal.node, (long long)msg->reply_to.cnt, (long long)msg->reply_to.node);
  //fflush(stdout);
  assert(p->proposer.msg);
  prepare_msg(msg);
...
/* 
  用于检测给定的synod是否超过本视图配置(在reconfigure state machine 论文中，state machine v)下的阈值
  新的视图变更配置可能会修改event horizon的值，通常情况下，在没有成员变更配置存在时，消息号的阀值为：
  last_executed_synod + event_horizon(active_config)

  但是如果新的成员变更请求R到来，那么上述阈值可能超出新的event_horizon。比如ast_executed_synod = 42，event_horizon(active_config) = 10.
  此时此成员的消息号最大为52，但是如果新的成员变更配置在45号消息生效，并且event_horizon被修改为2，那么此时节点最大可以使用的消息号为47，
  成员以前可能已经开始处理47到52之间指向synods的消息，但是现在将忽略指向这些synods的消息。

  我们不希望开始处理最终会脱离event_horizon范围外的消息。更重要的是，由于executor_task的退出逻辑，上述阈值可能不安全。

  当一个节点从synod start（C）开始从配置C上的组中移除自己时，
  退出逻辑依赖于知道何时大多数节点已执行了start（C）-1号消息，即包含离开节点的最后一个配置的最后一条消息。

  在event_horizon不变的情况下，我们知道当synod start（C）+event_horizon被learn到时，
  这是因为大多数已经执行或准备执行（已经被learne 到了）synod start（C）。这意味着大多数节点已经执行了start（C）-1 号消息了。

  对于动态的event_horizon，我们不能确定当学习到synod start（C）+event_horizon（C）时，
  大多数已经执行或准备去执行synod start（C）。这是因为一个新的、更小的event_horizon可能在start(C)和start（C）+event_horizon（C）之间生效。
  如果发生这种情况并且节点允许参与可能超出start（C）+event_horizon（C）的synods，
  这可能导致在没有大多数已经执行或准备执行synod start（C）的情况下学习synod start（C）+event_horizon（C）的值。

  为了维持executor_task的退出逻辑所做的假设，当成员配置变更R处于等待状态时，我们将阈值设置为以下两者之间的最小值：

  last_executed_synod + event_horizon(active_config) 和 start(R) - 1 + event_horizon(R)
...
static int proposer_task(task_arg arg) {
  ...
  while (!xcom_shutdown) { /* Loop until no more work to do */
    ...
    //printf("%lu", sizeof(prop_input_queue));
  ...
  retry_new:
    ...
    // printf("111111111\n");
    // printf("ep->msgno, %lld,   %lld\n",(long long)ep->msgno.msgno, (long long)ep->msgno.node);
    // printf("current_message, %lld,   %lld\n",(long long)current_message.msgno, (long long)current_message.node);
    // fflush(stdout);
    ...
    for (;;) { /* Loop until the client message has been learned */
      ...
      if (match_my_msg(ep->p->learner.msg, ep->client_msg->p)) {
        // printf("finsh one propose: message num:%lld, ballot num: %lld, server node num:%d\n", (long long)ep->p->synode.msgno,(long long)ep->p->proposer.bal.cnt,ep->p->proposer.bal.node);
...
void execute_msg(site_def *site, pax_machine *pma, pax_msg *p) {
  ...
  if (a) {
      ...
      //printf(" execute msg type:%d , execute is :%lld,   %lld\n",(int)a->body.c_t, (long long)p->synode.msgno, (long long)p->synode.node);
      //fflush(stdout);
    switch (a->body.c_t) {
...
/*
  在当前配置被提出和现在中间的时间段，并发的重新配置可能同时生效

  特别是，任何并发的重新配置都可能修改了event_horizon，并且新成员不支持event_horizon 重新配置。

  我们通过验证在当前状态下是否仍然可以添加新成员来解决这些情况。

  否则，此重新配置不会产生任何效果，即不会安装新配置。
*/
...
/*
  ...
  inform_removed（）会执行此操作。通过跟踪包含要离开的节点的最旧配置来处理配置足够接近C0<C1<=C0+alpha的情况。
  ...
  在这种情况下，B将在A有机会学习C2.start+alpha之前退出，这将使A永远都被hang住。
  显然，我们需要施加一个额外的约束，即C3.start必须大于C2.start+alpha。这由空配置的特殊测试来处理。

  除了上述方法之外，还有一个干净简单的解决方案尚未实施，因为它需要对共识逻辑进行更多的修改。
  如果我们要求消息C2..C2.start‐1的大部分来自C1中的节点和C2中的节点，
  ...
  由于接受者通过其节点号进行标识，并且两个配置的节点号不必相同，因此我们需要保持任意两个连续配置的节点号之间的映射。
  或者，我们可以删除节点号alto-gether，并始终为节点使用一个唯一的、不变的ID，如IP address+port。
...
static void dump_exec_state(execute_context *xc, xcom_dbg_type dbg);
...
static void dump_exec_state(execute_context *xc MY_ATTRIBUTE((unused)),
                            xcom_dbg_type dbg MY_ATTRIBUTE((unused))) {
...
static int sweeper_task(task_arg arg MY_ATTRIBUTE((unused))) {
  ...
  while (!xcom_shutdown) {
     ...
     */
    {
      while (synode_lt(ep->find, max_synode) && !too_far(ep->find)) {// 也就是如果当前消息的synode_no msgno大于本节点executed_msg+事件时间窗（too_far()）
        ...
        if (pm && !pm->force_delivery) { /* We want full 3 phase Paxos for
          ...
          if (!is_busy_machine(pm) && pm->acceptor.promise.cnt == 0 &&
              !pm->acceptor.msg && !finished(pm)) { //instance未被使用并且 没有在对应instance上做出承诺并且没有接受别的节点建议的消息或者自身未建议消息并且instance内有学习到任何值
            pm->op = skip_op;
            ADD_DBG(D_CONS,
...
bool_t handle_simple_ack_prepare(site_def const *site, pax_machine *p,
...

  {
    ...
    //printf(" sent_prop ---- op: %s, reply: %s, p cnt: %lld, p node: %lld, m cnt: %lld, m node: %lld\n", pax_op_to_str(p->op),pax_op_to_str(m->op),(long long)p->proposer.sent_prop.cnt, (long long)p->proposer.sent_prop.node, (long long)m->reply_to.cnt, (long long)m->reply_to.node);
...
static void handle_ack_prepare(site_def const *site, pax_machine *p,
  ...
  */
  ...
  //printf(" before ---- op: %s, reply: %s, p cnt: %lld, p node: %lld, m cnt: %lld, m node: %lld\n", pax_op_to_str(p->op),pax_op_to_str(m->op),(long long)p->proposer.bal.cnt, (long long)p->proposer.bal.node, (long long)m->reply_to.cnt, (long long)m->reply_to.node);
  //fflush(stdout);
...
static void handle_client_msg(pax_msg *p) {
  // printf(" %u, %u, %d, from %lu, to %u, %s\n", p->from ,p->to,p->msg_type, p->synode.msgno,p->synode.node, pax_op_to_str(p->op));
...

  {
    ...
    for (i = 0; i < nr_nodes_to_add; i++) {
      if (node_exists(&nodes_to_change[i], &new_site_def->nodes) ||
        ...
        /*
          我们只是忽略了在组中添加一个节点的尝试，当它有一个旧的化身时，这意味着该节点崩溃并重新启动得如此之快，以至于没有人注意到它已经消失了。
          在XCOM中，组不会自动重新配置，可以重新使用已崩溃并重新启动的节点，而无需通过将节点添加回该节点来重新配置该节点。
          但是，此操作可能不安全，因为XCOM没有实现崩溃恢复模型，并且节点在重新启动服务后会出现健忘症。
          换句话说，这可能会导致paxos协议中的不一致性问题。不幸的是，阻止将节点添加回存在旧版本的系统中并不能解决此问题，因为需要进行其他更改。
        */
        /*
...
pax_msg *dispatch_op(site_def const *site, pax_msg *p, linkage *reply_queue) {
  ...
  //printf(" from %u, to %u, msgno： %lu, node is：%u, msg type is %s, op type is %d\n", p->from ,p->to, p->synode.msgno,p->synode.node, pax_op_to_str(p->op), p->msg_type);
  //printf("msg type is:%s\n", pax_op_to_str(p->op), );
  //fflush(stdout);
  switch (p->op) {
    case client_msg:
      clicnt++;
...
char const *xcom_fsm(xcom_actions action, task_arg fsmargs) { // 蹦床函数：在基本情况下返回一个值，在其他情况下返回要调用的函数
