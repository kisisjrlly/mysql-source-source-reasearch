  ...
  if (hash_table != NULL) {
    ...
    FWD_ITER(bucket, pax_machine, {
      if (synode_eq(link_iter->synode, synode)) {
        ...
        return link_iter; // 经过强制转化后返回pax_machine类型
      }
    });
  }
  return NULL;
...
pax_machine *init_pax_machine(pax_machine *p, lru_machine *lru,
                              synode_no synode) {
  sub_cache_size(p); // 从整体的cache_size中减小 p所占的大小 
...
static void expand_lru() {
  ...
  for (i = 0; i < BUCKETS; i++) {
    ...
    link_into(&l->lru_link, &probation_lru);// 将将lru_machine的linkage节点链接到probation_lru的前面
