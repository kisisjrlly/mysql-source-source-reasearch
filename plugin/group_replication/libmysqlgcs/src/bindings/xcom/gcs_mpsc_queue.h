  ...
  bool push(T *payload) {
    ...
    if (new_node != nullptr) {
      Gcs_mpsc_queue_node *previous =
          m_head.exchange(new_node, std::memory_order_acq_rel); // atomic 变量成员函数 exchange，交换m_head 和 new_node, 返回m_head旧值
      previous->set_next(new_node, std::memory_order_release);  // 指定内存分配方式为memory_order_release，
      successful = true;
    }
    return successful;
  }
