 ...
 * Implements a tagged lock for optimistic read-side sections. 为乐观读取部分实现带标记的锁。
 *
 ...
 *
 * For the write-side section, one uses it as a typical spin lock, e.g.: 对于写端部分，可以将其用作典型的自旋锁
 *
 *     do:
 ...
 * any memory stores. //这样做的目的是允许乐观读取部分不执行任何内存存储。
 ...
 *       如果同时获取了标记锁，则标记锁的读取侧的执行可以与写入侧部分同时执行
 ...
 *       如果同时获取了标记锁，则标记锁的读取端可能会失败，在这种情况下，可能需要回滚失败的读取侧部分的效果
 *
