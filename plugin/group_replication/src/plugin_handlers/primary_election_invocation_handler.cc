...
int Primary_election_handler::execute_primary_election(
  ...
  for (Group_member_info *member : *all_members_info) {
    std::cout<< "uuid: " <<member->get_uuid()<<std::endl;
    std::cout<< "role: "<< member->get_role()<<std::endl;
    std::cout<< "group_name: "<< member->get_group_name()<<std::endl;
    std::cout<< "member_id(): " << member->get_gcs_member_id().get_member_id() <<std::endl;
    std::cout<< "member_weight: "<< member->get_member_weight()<<std::endl;
    std::cout<< "gtid_executed: " <<member->get_gtid_executed() << std::endl;
    std::cout<< "gtid_purged: " <<member->get_gtid_purged() << std::endl;
    std::cout<< "gtid_retrieved: " <<member->get_gtid_retrieved() <<std::endl;
    std::cout<<"++++++++++++++++++++++++++++++++++++"<<std::endl;
  }
  std::cout<<"--------------------------------------"<<std::endl;
...
bool Primary_election_handler::pick_primary_member(
   ...
   */
  sort_members_for_election(all_members_info, lowest_version_end, local_member_info->get_primary_election_self_adaption());
...
void sort_members_for_election(
  ...
  if (lowest_version >= PRIMARY_ELECTION_MEMBER_WEIGHT_VERSION) {
    ...
    std::cout<<"use gitd or not: "<<primary_election_self_adaption<<std::endl;
