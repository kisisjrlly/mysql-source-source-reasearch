  ...
  */
  ...
  if (local_member_info != nullptr) {
    local_member_info->update(
        hostname, ov.primary_election_self_adaption_var, ov.group_name_var, port, uuid, lv.write_set_extraction_algorithm,
...
static int check_primary_election_self_adaption(MYSQL_THD, SYS_VAR *, void *save,
  ...
  if (plugin_is_group_replication_running()) {
    mysql_mutex_unlock(&lv.plugin_running_mutex);
    my_message(ER_GROUP_REPLICATION_RUNNING,
               "The group_replication_primary_election_self_adaption cannot be turned ON/OFF when Group "
               "Replication is running",
               MYF(0));
    return 1;
  }
