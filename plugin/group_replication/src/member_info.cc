...
Group_member_info::Group_member_info(
    const char *hostname_arg, bool primary_election_self_adaption_arg,
    const char *group_name_arg, uint port_arg, const char *uuid_arg,
...
void Group_member_info::update(Group_member_info &other) {
  ...
  update(
      other.get_hostname().c_str(), other.get_primary_election_self_adaption(),
      other.get_group_name().c_str(), other.get_port(), other.get_uuid().c_str(),
      other.get_write_set_extraction_algorithm(),
...
bool Group_member_info::get_primary_election_self_adaption(){
...
bool Group_member_info::has_greater_weight(Group_member_info *other) {
  ...
  // size_t this_pos_of_colon = executed_gtid_set.find_first_of(':');
  // size_t other_pos_of_colon = other->executed_gtid_set.find_first_of(':');

  // assert(this_pos_of_colon == this_pos_of_colon);

  /* 
    format of executed_set: [group_name:gtid_set,group_name:gtid_set] eg. [aaaa:1-4:5:6, bbbbb:1-10:11:12] 
    lambda function to split executed_set to map: group_name->gtid_set
  */
  //std::string group_name = other->get_group_name();
  ...
  // map<std::string, std::string> gtidset_map_this = Split(str, ",");
  ...
  // printf("%s---:%s---:%lld\n",uuid.c_str(), gitdset_this.c_str(), n2);
  // printf("%s---:%s---:%lld\n",other->get_uuid().c_str(), gitdset_this.c_str(), n2);
  // printf("/////////////2////////////// \n");
...
bool Group_member_info::has_greater_executed_gtid(Group_member_info *other){
  ...
  char *buf1 = new char[executed_gtid_set.size()+1];
  char *buf2 = new char[other->get_gtid_executed().size()+1];

  this_Gtid_set.to_string(buf1);
  other_Gtid_set.to_string(buf2);

  //std::cout<<*buf<<std::endl;
  printf("-----------has_greater_executed_gtid begin-----------\n \n \n");
  //std::cout<< "member_id(): " << member->get_gcs_member_id().get_member_id() <<std::endl;
  printf("this member id: %s\n", get_gcs_member_id().get_member_id().c_str());
  printf("other member id: %s\n", other->get_gcs_member_id().get_member_id().c_str());
  printf("this executed gtid: %s\n",executed_gtid_set.c_str());
  printf("other executed gtid: %s\n",other->get_gtid_executed().c_str());
  printf("this_Gtid_set: %s\n", buf1);
  printf("other_Gtid_set: %s\n", buf2);

  if(other_Gtid_set.is_subset_not_equals(&this_Gtid_set)) {
    printf("other_Gtid_set is subset of and not equal to this_Gtid_set\n\n\n");
    printf("-----------has_greater_executed_gtid end-----------\n \n \n");
    fflush(stdout);
    return true;
  }

  if(this_Gtid_set.is_subset_not_equals(&other_Gtid_set)) {
    printf("this_Gtid_set is subset of and not equal to other_Gtid_set\n\n\n");
    printf("-----------has_greater_executed_gtid end-----------\n \n \n");
    fflush(stdout);
    return false;
  }
  ...
  printf("other_Gtid_set is equal to this_Gtid_set\n\n\n");
  printf("-----------has_greater_executed_gtid end-----------\n \n \n");
  fflush(stdout);
  if (member_weight > other->get_member_weight()) return true;
