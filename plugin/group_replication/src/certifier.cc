...
int Certifier::handle_certifier_data(
  ...
  // if (executed_set.add_gtid_encoding(payload, packet->len) !=
  //       RETURN_STATUS_OK) {
  //     LogPluginErr(ERROR_LEVEL,
  ...
  // std::vector<Group_member_info *> member_all  = *(group_member_mgr->get_all_members());
  // std::vector<Group_member_info *>::iterator it;
  // for(it = member_all.begin(); it != member_all.end();it++){
  //   if((*it)->get_gcs_member_id() == gcs_member_id){
  //     (*it)->update_gtid_sets()
  //   }
  // }
  ...
  */
  ...
  if (this->members.size() != number_of_members_online) {
    ...
    */
    //std::string s1(*data,len);
    //std::cout<<"isisisisi: "<<s1<<std::endl;
    //std::fflush(stdout);
...
int Certifier::stable_set_handle() {
  ...
  */
  while (!error && !this->incoming->empty()) {
    ...
    //std::cout<<"test::"<<*payload<<std::endl;
    //fflush(stdout);
